...
async def on_ready(user):
    ...
    if CHANNEL_ID == "YOUR_CHANNEL_ID_HERE":
        print("Please replace 'YOUR_CHANNEL_ID_HERE' with your actual channel ID!")
        await client.close()
        return
    ...
    await client.send_message(
        CHANNEL_ID, "Here is a file sent via path:", files=["example_file.txt"]
    )
    ...
    await client.send_message(
        CHANNEL_ID, "Here is a file sent via BytesIO:", files=[("memory_file.txt", fp)]
    )
